...
double vector_standard_deviation(int size, double *vector, int option){
    ...
    // if the option 1 it will be sample
    // if the option 0 it will be population
    double V_variance = vector_variance(size, vector, option); // the vector variance
...
double vector_hadamard_product_sum(int size, double *vectorA, double *vectorB){
    ...
    double V_product_sum = 0.0;
    
    for(int i=0; i<size; i++){
        V_product_sum += vectorA[i] * vectorB[i];
    }
...
void vector_abs(int size, double *vector, double *result_vectors){
    if(size<=0 || vector == NULL || result_vectors) return; // undefined
    
    for(int i=0; i<size; i++){
        result_vectors[i] = fabs(result_vectors[i]);
    }
    
}

void vector_negate(int size, double *vector, double *result_vectors){
    if(size<=0 || vector == NULL || result_vectors == NULL)return; // undefined 
...
void vector_sqrt(int size, const double *vector, double *result_vectors){
    ...
    for(int i=0; i<size; i++){
        if (vector[i] < 0.0) {
            result_vectors[i] = 0.0; // inserting 0 if the vector negative
        } else {
...
int main(){
    ...
    printf("\n==========vector_variance==============\n");
    printf("%lf", vector_variance(size, arr, 0));
    
    printf("\n======= vector_reverse ======= \n");
    vector_reverse(size, arr, result);
    display(size, result);
    
    
    printf("\n============vector_sort_ascending=================\n");
    vector_sort_ascending(size,arr, result);
    display(size, result);
    
    printf("\n=================vector_sort_descending==================\n");
    vector_sort_descending(size, arr, result);
    display(size, result);
